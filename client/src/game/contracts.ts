...
export type WeaponId = "assault" | "smg" | "shotgun";
export type WeaponDefinition = { id: WeaponId; name: string; damage: number; magazineSize: number; fireInterval: number; range: number; ammoType: "medium" | "light" | "shells"; pellets: number };
...
export type HudSnapshot = { hp: number; maxHp: number; shield: number; ammo: number; reserve: number; remaining: number; elims: number; zone: string; motion: MotionState; aiming: boolean; crouching: boolean; pickup: string; switchPrompt?: boolean; objective?: string; bossName?: string; bossHp?: number; bossMaxHp?: number; weaponName?: string; slots?: Array<WeaponId | null>; medkits?: number };
